...
export function getCatchupInfo(channel: Channel): CatchupInfo {
  ...
  let catchupAvailable = !!(channel.catchup && channel.timeshift && channel.timeshift > 0)
  ...
  if (channel.url) {
    ...
    if (urlLower.includes('cdnx1.plusx.tv') || urlLower.includes('plusx.tv')) {
      console.log('🔄 EXTENDED: Detected plusx.tv provider, using 5 days catchup (120h)')
    ...
    // Check for other itvn.io providers
      ...
      console.log('🔄 FALLBACK: Detected itvn.io provider, assuming 10h catchup')
...
function generateFlussStreamsCatchupUrl(baseUrl: string, utcTime: number): string {
...
export function isTimeWithinCatchup(channel: Channel, time: Date): boolean {
  ...
  console.log('Checking catchup availability for:', {
  ...
  if (time > catchupInfo.endTime) {
    console.log('Time is in the future, cannot catchup')
    return false
  }
...
export function getCatchupTimeRange(channel: Channel): { start: Date; end: Date } | null {
  const catchupInfo = getCatchupInfo(channel)
...
export function getTimeFromRelativePosition(channel: Channel, position: number): Date {
  const range = getCatchupTimeRange(channel)
